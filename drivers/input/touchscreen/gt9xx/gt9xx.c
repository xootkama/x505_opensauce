...
#define GTP_PEN_BUTTON1		0x298 //BTN_STYLUS
#define GTP_PEN_BUTTON2		0x299 //BTN_STYLUS2
...
int gtp_i2c_read(struct i2c_client *client, u8 *buf, int len)
{
	...
	len -= GTP_ADDR_LENGTH;
	//GTP_INFO("gtp enter fun:%s,line:%d\n",__func__,__LINE__);
...
static int gtp_gesture_handler(struct goodix_ts_data *ts)
{
	...
	if (doze_buf[2] == 0xCC) {
		...
		input_report_key(ts->input_dev, KEY_GTP_OPEN, 1);
		input_sync(ts->input_dev);
		input_report_key(ts->input_dev, KEY_GTP_OPEN, 0);
		input_sync(ts->input_dev);
		/*  clear 0x814B */
...
static void gtp_pen_init(struct goodix_ts_data *ts)
{
	...
	input_set_abs_params(ts->pen_dev, ABS_MT_POSITION_X, 0,
			     ts->pdata->abs_size_x, 0, 0);
	input_set_abs_params(ts->pen_dev, ABS_MT_POSITION_Y, 0,
			      ...
			      struct goodix_point_t *points)
{
	...
	for (i = 0; i < ts->pdata->max_touch_id; i++) {
		if (touch_num && i == points->id) {
			GTP_DEBUG("andy enter type report down\n");
			       ...
			       struct goodix_point_t *points)
{
	...
	for (i = 0; i < ts->pdata->max_touch_id; i++) {
		if (touch_num && i == points->id) {
					...
					if(pre_touch) {
						...
						for (i = 0; i < ts->pdata->max_touch_id; i++) {
							/**********gtp_touch_up(ts, i);***************/
							 ...
							 input_sync(ts->input_dev);
							 GTP_DEBUG("hoverfuc,finger up.\n");
						}
					}
				/**	gtp_hover_down(input_x, input_y, input_w, 0);**/
					input_report_key(ts->pen_dev, BTN_TOOL_PEN, 1);
					input_mt_slot(ts->pen_dev, points->id);
					input_report_abs(ts->pen_dev, ABS_MT_POSITION_X, points->x);
					input_report_abs(ts->pen_dev, ABS_MT_POSITION_Y, points->y);
					input_report_key(ts->pen_dev, BTN_TOUCH, 0);

					input_mt_sync_frame(ts->pen_dev);
					input_sync(ts->pen_dev);

					//GTP_DEBUG("hover_down =%s,line:%d\n",__func__,__LINE__);
					hover_state = 1;
				} else {
					if(hover_state) {
						
						/****gtp_hover_up(input_x, input_y, input_w, 0);******/
			...
			if(hover_state&&!touch_num){
				//GTP_DEBUG("hover release =%s,line:%d\n",__func__,__LINE__);
				hover_state = 0;
				/****gtp_hover_up(input_x, input_y, input_w, 0);******/
				input_report_key(ts->pen_dev, BTN_TOOL_PEN, 0);
				input_mt_slot(ts->pen_dev, points->id);
...
static int gtp_find_valid_cfg_data(struct goodix_ts_data *ts)
{
	...
	/* if defined GTP_CONFIG_OF, parse config data from dtsi
...
s32 gtp_read_version(struct i2c_client *client, u16* version)
{
    ...
    if (buf[5] == 0x00)
    {
    	//GTP_DEBUG("andy added test 1 !!!!!!!!!buf[5]=%02x\n",buf[5]);
        GTP_INFO("IC Version: %c%c%c_%02x%02x", buf[2], buf[3], buf[4], buf[7], buf[6]);
        strlcpy(product_id, &buf[2], GTP_PRODUCT_ID_MAXSIZE - 1);
    }
    else
    {
    	//GTP_DEBUG("andy added test 2 !!!!!!!!!buf[5]=%02x\n",buf[5]);
...
//GTP_INFO("gtp enter fun:%s,line:%d\n",__func__,__LINE__);
	...
	ret = gtp_i2c_read_dbl_check(client, GTP_REG_SENSOR_ID,
				     &fw_info->sensor_id, 1);
	    GTP_INFO("Sensor_ID: %d", fw_info->sensor_id);
    
#ifdef SUPPORT_READ_TP_VERSION
	g_sensor_id = fw_info->sensor_id;//Bert
	GTP_DEBUG("%s: Sensor_ID: %d\n", __func__, fw_info->sensor_id);
#endif
...
//GTP_DEBUG("andy enter input_dev line=%d\n!!!!!!",__LINE__);
	...
	if (ts->pdata->slide_wakeup){
		input_set_capability(ts->input_dev, EV_KEY, KEY_GTP_OPEN);
		}
	if (ts->pdata->swap_x2y)
...
static int gtp_power_switch(struct i2c_client *client, int on)
{
	...
	if (on) {
		...
		if (gpio_is_valid(power_ldo_gpio)) {
			GTP_DEBUG("%s, power_ldo_gpio\n", __func__);
/*			ret = gpio_request(power_ldo_gpio, "gt9xx_ldo_gpio");
			if (ret) {
				GTP_DEBUG("gt9xx power ldo gpio request failed\n");
				return ret;
			}
*/		
			ret = gpio_direction_output(power_ldo_gpio, 1);
			if (ret) {
				GTP_DEBUG("gt9xx set_direction for power ldo gpio failed\n");
				goto free_ldo_gpio;
			}
		}
...
{
	int ret = 0;
	GTP_DEBUG("enter gtp_power_off func \n");
...
static void gtp_charger_config_check(s32 dir_update)
{
	...
	if (g_charger_present){
		//GTP_DEBUG("andy test g_charger_present = %d\n",g_charger_present);
		if (!chr_pluggedin || dir_update) {
			chr_cmd[2] = 6;
			GTP_INFO("Update status for Charger Plugin");
/* LXF_P400_A01-50 add by panfei 20180919  begin */
		...
		if (chr_pluggedin || dir_update) {
			...
			for (count = 0; count < 5; count++)
            {
				...
				if((chr_state[2]&0x01)!=1)
					count = 5;
				GTP_INFO("2chr_state[2] = %d\n",chr_state[2]);
			}
			chr_pluggedin = 0;
			//gtp_esd_switch(ts->client, SWITCH_ON);
/* LXF_P400_A01-50 add by panfei 20180919  end */
		}
	}
}
...
//---#endif
...
#ifdef SUPPORT_READ_TP_VERSION
	GTP_INFO("version_info=0x%02X, product_id=%s", version_info,  product_id);
	...
	else if(g_sensor_id == 1)
	{
		init_tp_fm_info(0, fw_version, "Mutto");
	}
	else
	{
...
static void gtp_suspend(struct goodix_ts_data *ts)
{
	...
	gtp_esd_off(ts);
	gtp_work_control_enable(ts, false);
	if (ts->pdata->slide_wakeup && gesture_enable_flag) {
...
static void gtp_resume(struct goodix_ts_data *ts)
{
	...
	if (ts->pdata->slide_wakeup && test_bit(DOZE_MODE, &ts->flags)&&gesture_suspend_resume_flag) {
...
static int __init gtp_init(void)
{
	...
	char *tp_p=NULL;
	tp_p = NULL;
	tp_p= strstr(boot_command_line,"androidboot.goodixtp=gtp");
	GTP_INFO("andy tp_p = %s",tp_p);
	if(!tp_p){
	return -1;
	}
#if GTP_CHARGER_SWITCH
