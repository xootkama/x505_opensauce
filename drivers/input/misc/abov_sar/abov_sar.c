		...
		const char *buf, size_t count)
{
	...
	input_report_key(input_top, pDevice->pbuttonInformation->buttons[0].keycode_release, 1);
	input_sync(input_bottom);
	//msleep(1);
	input_report_key(input_top, pDevice->pbuttonInformation->buttons[0].keycode_release, 0);
	input_sync(input_bottom);
	pDevice->pbuttonInformation->buttons[0].state = IDLE;

	input_report_key(input_bottom, pDevice->pbuttonInformation->buttons[1].keycode_release, 1);
	input_sync(input_top);
	//msleep(1);
	input_report_key(input_bottom, pDevice->pbuttonInformation->buttons[1].keycode_release, 0);
	input_sync(input_top);
...
static int abov_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (this) {
		...
		if (pDevice) {
			...
			__set_bit(pDevice->pbuttonInformation->buttons[0].keycode, input_top->keybit);
			__set_bit(pDevice->pbuttonInformation->buttons[0].keycode_release, input_top->keybit);
			pDevice->pbuttonInformation->buttons[0].state = IDLE;
			...
			__set_bit(pDevice->pbuttonInformation->buttons[1].keycode, input_bottom->keybit);
			__set_bit(pDevice->pbuttonInformation->buttons[1].keycode_release, input_bottom->keybit);
			pDevice->pbuttonInformation->buttons[1].state = IDLE;
            /* save the input pointer and finish initialization */
...
{
	pabovXX_t this = i2c_get_clientdata(to_i2c_client(dev));
...
const struct dev_pm_ops abov_pm_ops = {
