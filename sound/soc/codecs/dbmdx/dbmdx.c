...
static int dbmdx_va_ve_set_asrp_gain(struct dbmdx_private *p, u32 gain_type)
{
	...
	if ((gain_type == ASR_OUTPUT_GAIN) &&	p->pdata->asrp_gain > 0) {
		cur_gain = (u16)(p->pdata->asrp_gain);
		gain_sel = DBMDX_REGV_ASRP_BFPF_OUT_1;
	} else if ((gain_type == VC_OUTPUT_GAIN) &&	p->pdata->vc_gain > 0) {
		cur_gain = (u16)(p->pdata->vc_gain);
		gain_sel = DBMDX_REGV_ASRP_VCPF_OUT_1;
	} else {
		return 0;
	}
...
#if defined(VA_VE_TDM_RX_MASTER)
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	off += snprintf(buf, PAGE_SIZE, "ASRP Gain: 0x%x\n",
		p->pdata->asrp_gain);

	off += snprintf(buf + off, PAGE_SIZE, "VC Gain: 0x%x\n",
		p->pdata->vc_gain);
					...
					const char *buf, size_t size)
{
	...
	if (val & 0x10000) {
		...
		p->pdata->vc_gain = val & 0xFFFF;

		dev_info(p->dev, "%s: VC Gain was set to 0x%x\n", __func__,
			p->pdata->vc_gain);

	} else {

		p->pdata->asrp_gain = val;

		dev_info(p->dev, "%s: ASRP Gain was set to 0x%x\n", __func__,
			p->pdata->asrp_gain);
	}
...
static int dbmdx_va_chip_probe(struct dbmdx_private *p)
{
	...
	ret = gpio_request(33, "1.2V LDO");
	if (ret < 0) {
		dev_err(p->dev, "%s: error requesting 1.2V LDO gpio\n", __func__);
		goto err;
	}
	gpio_direction_output(33, 1);

	ret = gpio_request(40, "1.1V LDO");
	if (ret < 0) {
		...
		struct dbmdx_private *p)
{
	...
	ret = of_property_read_u32(np, "asrp_gain",
		&p->pdata->asrp_gain);
	if (ret && ret != -EINVAL) {
		dev_err(p->dev, "%s: invalid 'asrp_gain'\n", __func__);
		ret = -EINVAL;
		goto out_err;
	} else if (ret ==  -EINVAL) {
		dev_info(p->dev,
			"%s: no asrp_gain definition in device-tree.\n",
			__func__);
		p->pdata->asrp_gain = 0;
	} else {
		dev_info(p->dev,
		"%s: using asrp_gain of 0x%x from device-tree\n",
			 __func__, p->pdata->asrp_gain);
	}
...
/* check if enabled features make sense */
	...
	dev_info(dev, "%s: using asrp_gain of 0x%8x\n",
		 __func__, pdata->asrp_gain);
